        ...
        if (pos_min >= pos_min_thold || is_dsv4 || is_openpangu) {
            ...
            if (!do_reset) {
                ...
                if (!do_reset) {
                    ...
                    // (mirrors ggml-org#2432 direction; stale offsets poison chained restores).
                    slot.n_past_offset = slot.n_past_prompt - slot.n_past;
                    SLT_WRN(slot, "post-restore align: n_past = %d, n_past_prompt = %d, n_past_offset = %d (ckpt pos_max = %d)\n",
                        slot.n_past, slot.n_past_prompt, slot.n_past_offset, it->pos_max);
